...
public class Enemy05 extends Entity {
	...
	private int life = 300, maxLife=150, xpEnemy = 250, idEnemy = 5, Damaged = 20;

	public Enemy05(int x, int y, int width, int height, BufferedImage sprite) {
		...
		if ((calculeteDistance(this.getX(), Game.player.getX(), this.getY(), Game.player.getY()) < 60 || life < 100)) {
			if (!this.isColiddingWithPlayer()) {
				if ((int) x < Game.player.getX() && World.isfree((int) (x + speed), this.getY())
						&& !isColiddingEnemies5((int) (x + speed), this.getY())) {
					moved = true;
					dir = right_dir;
					x += speed;
				...
				if ((int) y < Game.player.getY() && World.isfree(this.getX(), (int) (y + speed))
						&& !isColiddingEnemies5(this.getX(), (int) (y + speed))) {
					moved = true;
					y += speed;
