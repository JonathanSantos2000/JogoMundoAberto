...
public class Enemy04 extends Entity {
	...
	private int life = 250, xpEnemy = 200, idEnemy = 4,Damaged=15;

	public Enemy04(int x, int y, int width, int height, BufferedImage sprite) {
		...
		if ((calculeteDistance(this.getX(), Game.player.getX(), this.getY(), Game.player.getY()) < 60 || life < 100)) {
			if (!this.isColiddingWithPlayer()) {
				if ((int) x < Game.player.getX() && World.isfree((int) (x + speed), this.getY())
						&& !isColiddingEnemies4((int) (x + speed), this.getY())) {
					moved = true;
					dir = right_dir;
					x += speed;
				...
				if ((int) y < Game.player.getY() && World.isfree(this.getX(), (int) (y + speed))
						&& !isColiddingEnemies4(this.getX(), (int) (y + speed))) {
					moved = true;
					y += speed;
