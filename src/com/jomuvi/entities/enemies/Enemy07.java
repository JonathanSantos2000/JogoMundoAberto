...
public class Enemy07 extends Entity {
	...
	private int life = 100, xpEnemy = 250, idEnemy = 7, Damaged = 20;

	public Enemy07(int x, int y, int width, int height, BufferedImage sprite) {
		...
		if ((calculeteDistance(this.getX(), Game.player.getX(), this.getY(), Game.player.getY()) < 60 || life < 100)) {
			...
			if (!this.isColiddingWithPlayer()) {
				if ((int) x < Game.player.getX() && World.isfree((int) (x + speed), this.getY())
						&& !isColiddingEnemies7((int) (x + speed), this.getY())) {
					moved = true;
					dir = right_dir;
					x += speed;
				...
				if ((int) y < Game.player.getY() && World.isfree(this.getX(), (int) (y + speed))
						&& !isColiddingEnemies7(this.getX(), (int) (y + speed))) {
					moved = true;
					y += speed;
