...
public class NPCStoreStyler extends Entity {
	public String[] frases = new String[4];

	// fim

	private BufferedImage NPCStoreStyler;
	private boolean ShowTexto = false;

	public NPCStoreStyler(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, sprite);
		frases[0] = "Olá. Bem vindo! Ao Shopping";
		frases[1] = "no momento não estamos aberto.";
		frases[2] = "volte mais tarde (working in progress)";
		NPCStoreStyler = Game.spritesheetNPC.getSprite(05 * 16, 00 * 00, 16, 16);
	...
	public void render(Graphics g) {
		super.render(g);
		g.drawImage(NPCStoreStyler, this.getX() - Camera.x, this.getY() - Camera.y, null);
